...
{// Constractor
    ...
    this.Archer = loadImage(filename); // Load image of archer to program
    this.x = x;
    this.y = y;
    this.width = width;
    this.height = height;
  ...
  void update(float mouseY) 
{
  ...
  void displayArcher() 
{
    image(Archer, x, y); // display Archer photo 
}
  
  void setImage(String imagePath) 
{// this method make us to change image of archer when we clicked on mouse
    this.Archer = loadImage(imagePath); // this change archer photo with one I will pass
    this.Archer.resize(int(width), int(height)); // Resize the image
}
}
